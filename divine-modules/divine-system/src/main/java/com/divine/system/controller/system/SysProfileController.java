...
import com.divine.system.domain.dto.SysUserProfileDto;
import com.divine.common.core.utils.StringUtils;
import com.divine.common.core.utils.file.MimeTypeUtils;
...
public class SysProfileController extends BaseController {
    ...
    public Result<Void> updateProfile(@Validated @RequestBody SysUserProfileDto profile) {
        SysUserDto user = BeanUtil.toBean(profile, SysUserDto.class);
        user.setUserId(LoginHelper.getUserId());
        String username = LoginHelper.getUsername();
        if (StringUtils.isNotEmpty(user.getPhonenumber()) && !userService.checkPhoneUnique(user)) {
            return Result.fail("修改用户'" + username + "'失败，手机号码已存在");
        }
        if (StringUtils.isNotEmpty(user.getEmail()) && !userService.checkEmailUnique(user)) {
            return Result.fail("修改用户'" + username + "'失败，邮箱账号已存在");
        }
    ...
    /**
     * 重置密码
     * SaCheckRole：防止线上用户随意更改密码，用户可以将其注释掉
     *
    ...
    public Result<Void> updatePwd(String oldPassword, String newPassword) {
        SysUserVo user = userService.selectUserById(LoginHelper.getUserId());
